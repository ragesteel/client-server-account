...
Во, оно теперь ещё и работает. Дальше бы уже конечно совершенствовать, например добавить счётчик количества запросов в секунду. Тут по идее просто, заводим в менеджере счётчик который запросы считает. И сбрасываем её каждую новую секунду. Можно даже в обновлялщике сделать. Вычислять изменение во времени, на этом основании считать скорость и сбрасывать счётчик. Вернее получение и сброс будет в одном методе.

Что-же касается интерфейса — тут бы конечно разделить контроллеры — выделить общую часть по управлению соответствующим потоком.
